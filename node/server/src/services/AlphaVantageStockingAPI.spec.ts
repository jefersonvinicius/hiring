...
describe("AlphaVantageStockingAPI", () => {
  ...
  describe("fetchByName", () => {
    it("should return an stock instance", async () => {
      const apiGetSpy = jest.spyOn(alphaVantageApi, "get").mockResolvedValue(createValidQueryResponse());
    ...
    it("should returns null when not return stock info", async () => {
      jest.spyOn(alphaVantageApi, "get").mockResolvedValue(createQueryResponseEmpty());
...
function createValidQueryResponse() {
  return {
    data: {
      "Global Quote": {
        "01. symbol": "any_stock",
...
function createQueryResponseEmpty() {
  return {
    data: {
      "Global Quote": {},
    },
